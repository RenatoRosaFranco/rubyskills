...
RSpec.describe RubySkills::Installer do
  ...
  let(:manifest) do
    instance_double(RubySkills::Manifest, skills: [skill]).tap do |double|
      allow(double).to receive(:load!).and_return(double)
    end
  end

  before do
    allow(RubySkills::Manifest).to receive(:new).and_return(manifest)
  end
